...
class EncoderRegressionModel(chainer.Chain):
    def __init__(self, in_size=[1, 2], encoder_n_units=10, regression_n_units=10, common_out_size=10):
        ...
        with self.init_scope():
            ...
            # Regression 2
            self.conv = links.ConvolutionND(ndim=1, in_channels=1, out_channels=regression_n_units, ksize=common_out_size,
...
if __name__ == '__main__':
    ...
    for index in range(10):
        for target in range(len(list_data)):
            ...
            measure_list = list()
            tmp_data = list_data.copy()
            tmp_data.remove(list_data[target])
            for source in tmp_data:
                measure_item = main(encoder_n_units=32, common_size=16, regression_n_units=32, discriminator_n_units=32,
                                    batch_size_def=[batch_size[target], batch_size[target]], epoch=5000,
                                    data_set_name=[source, list_data[target]], train_size=0.7, save_code=False,
                                    validation_patience_original=patience[target])
                measure_list.append(measure_item)
            measure_list = np.array(measure_list)
            measure_median = np.median(measure_list, axis=0)
            time_end = time.time()
            measure_median = np.append(measure_median, time_end - time_start)
            r[list_data[target]] = measure_median
        write_string = ''
        for key, value in r.items():
            ...
            print(key, "time consuming=", value[4])
            ...
            write_string += key + ":time consuming=" + str(value[4]) + '\n'
