...
if __name__ == '__main__':
    ...
    # -----------the structure of result{target1: {
    ...
    for target in range(len(list_data)):
        ...
        for source in range(len(source_data)):
            if is_first:
                best_test = main(encoder_n_units=32, common_size=16, regression_n_units=32,
                                             ...
                                             validation_patience_original=source_patience[source])
                key_str += (","+source_data[source])
                target_result[key_str] = best_test
                is_first = False
            else:
                best_test = main(encoder_n_units=32, common_size=16, regression_n_units=32,
                                             discriminator_n_units=32,
